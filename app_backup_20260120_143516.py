...
def update_user_settings():
    ...
    settings.sms_notifications = data.get('smsNotifications', settings.sms_notifications)
    settings.leave_requests = data.get('leaveRequests', settings.leave_requests)
    settings.payroll_alerts = data.get('payrollAlerts', settings.payroll_alerts)
    settings.attendance_reminders = data.get('attendanceReminders', settings.attendance_reminders)
    settings.two_factor_auth = data.get('twoFactorAuth', settings.two_factor_auth)
    settings.session_timeout = int(data.get('sessionTimeout', settings.session_timeout))
...
def create_notification(user_id, notif_type, title, message, icon='ℹ️', 
...
# ==============================================================================
# PAYSLIP PDF GENERATION ENDPOINT
# ==============================================================================
...
def generate_payslip_pdf(payroll_id):
    ...
    # Company Header
    elements.append(Paragraph("GLIMMER HRMS", title_style))
    ...
    emp_data = [
        ['Employee Information', ''],
        ['Name:', employee.name],
        ['Employee ID:', employee.national_id],
        ['Department:', employee.department or 'N/A'],
        ['Position:', employee.position or 'N/A'],
        ['', ''],
        ['Payroll Period', ''],
        ['From:', payslip.period_start.strftime('%B %d, %Y')],
        ['To:', payslip.period_end.strftime('%B %d, %Y')],
    ]
    ...
    emp_table.setStyle(TableStyle([
        ...
        ('BACKGROUND', (0, 6), (-1, 6), colors.HexColor('#764ba2')),
    ...
    fin_table.setStyle(TableStyle([
        ...
        ('BACKGROUND', (0, 10), (-1, 10), colors.HexColor('#48bb78')),
    ...
    filename = f"Payslip_{employee.name.replace(' ', '_')}_{payslip.period_start.strftime('%b_%Y')}.pdf"
